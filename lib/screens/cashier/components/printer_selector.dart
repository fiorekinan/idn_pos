...
class PrinterSelector extends StatelessWidget {
  ...
  final BluetoothDevice? selectedDevices;
  final bool isConnected;
  final Function(BluetoothDevice?) onSelected;

  const PrinterSelector({super.key, required this.devices, this.selectedDevices, required this.isConnected, required this.onSelected});
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<BluetoothDevice>(
                ...
                items: devices.map((e) => DropdownMenuItem(
                  value: e,
                  child: Text(e.name ?? "Unknwon Device", style: TextStyle(fontSize: 14)),
                )).toList(),
